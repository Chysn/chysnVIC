...
START:  PHP
        ...
        TXA
        CLC
        ADC #$2B        ; The CHAIN vector is 43 ($2B) bytes after the JSR,
        STA SCRPAD      ; so add 43 to the low byte of the return address
        TYA
        ADC #$00        ; The high byte is only going to change if the
        STA SCRPAD + 1  ; carry flag was set with ADC #$2D above
        
        LDY #$00
        LDA IRQV        ; The idea here is that multple IRA routines can be
        STA (SCRPAD),Y  ; installed into memory, with each one "chaining" to
        INY             ; the next. This block of code looks up the current IRQ
        LDA IRQV + 1    ; location, and sets it as the location of the JMP
        STA (SCRPAD),Y  ; at the CHAIN label below. Your handler should branch to
                        ; CHAIN when it's finished.
